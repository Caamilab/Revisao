...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  // Verifica se é um número e deve ser desenhado em tamanho grande
  if (c >= '0' && c <= '9') {
    // Desenha um número grande (16x16 pixels)
    uint8_t *big_digit = NULL;
    
    // Seleciona o array do dígito correspondente
    switch(c) {
      case '0': big_digit = big_zero; break;
      case '1': big_digit = big_one; break;
      case '2': big_digit = big_two; break;
      case '3': big_digit = big_three; break;
      case '4': big_digit = big_four; break;
      case '5': big_digit = big_five; break;
      case '6': big_digit = big_six; break;
      case '7': big_digit = big_seven; break;
      case '8': big_digit = big_eight; break;
      case '9': big_digit = big_nine; break;
    }
    
    // Desenha o número grande (16x16 pixels)
    if (big_digit != NULL) {
      for (uint8_t i = 0; i < 16; ++i) {
        uint8_t line1 = big_digit[i * 2];       // Primeiro byte da linha
        uint8_t line2 = big_digit[i * 2 + 1];   // Segundo byte da linha
        
        for (uint8_t j = 0; j < 8; ++j) {
          // Desenha os primeiros 8 pixels da linha
          uint8_t pixel_on1 = (line1 >> (7 - j)) & 1;
          ssd1306_pixel(ssd, x + j, y + i, pixel_on1);
          
          // Desenha os próximos 8 pixels da linha
          uint8_t pixel_on2 = (line2 >> (7 - j)) & 1;
          ssd1306_pixel(ssd, x + j + 8, y + i, pixel_on2);
        }
      }
      return; // Sai da função após desenhar o número grande
    }
  }

  else if (c == ':') {
    // Desenha um ":" grande (16x16 pixels)
    for (uint8_t i = 0; i < 16; ++i) {
        uint8_t line1 = big_colon[i * 2];       // Primeiro byte da linha
        uint8_t line2 = big_colon[i * 2 + 1];   // Segundo byte da linha
    ...
    return; // Sai da função após desenhar o ":" grande
}
    ...
    index = (c - 'A' + 1) * 8; // Para letras maiúsculas
        ...
        // Caractere não suportado, use o espaço vazio
        index = 0;
    }
